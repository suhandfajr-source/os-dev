...
CONTEXT: Story 3 "Wizard Planning — Brief ke draf PRD" dari spec Meja Kendali. Alur: brief disimpan dulu sebagai artifact type=brief (selalu draft) → AI (Gemini, tier berat, JSON via responseMimeType) mendraf PRD → parser harden (strip fence, validasi field) → artifact type=prd draft (upsert) → user edit (PATCH) → approve (status approved; derivasi status planning menjadi "selesai"). Regenerate setelah approve = 409. Approve ulang = 200 tanpa mengubah updated_at. UI wizard di /dashboard/[projectId] dengan blok Pertanyaan Kritis menonjol + tombol disabled saat generate.
